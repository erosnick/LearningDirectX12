...
void shapesApp::draw(float delta) {
    ...
    if (isWireframe) {
        ThrowIfFailed(commandList->Reset(
            commandAllocator.Get(), pipelineStateObjects["opaque_wireframe"].Get()));
    }
    else {
        ThrowIfFailed(commandList->Reset(commandAllocator.Get(), pipelineStateObjects["opaque"].Get()));
    }
...
void shapesApp::createPipelineStateOjbect() {
    ...
    pipelineStateObjects["opaque"] = opaquePipelineStateObject;
    ...
    pipelineStateObjects["opaque_wireframe"] = opaqueWireframePipelineStateObject;
}
